...
def get_intersection(
    sp: Space, ix: int, iy: int, iz: int, idx: int, idy: int, idz: int
) -> Optional[Tuple]:
...
def split_space(
    sp: Space, rx: int, ry: int, rz: int, rdx: int, rdy: int, rdz: int
) -> List[Space]:
    res = []
    s = sp
    if rz - s.z > 0:
...
class BlockPacker:
    ...
    def try_place(
        self, name: str, dx: int, dy: int, dz: int
...
# 工件池生成
def generate_pool(
    rng: random.Random, extra_factor: float = 0.7
...
# 优先排列
def compute_priority(item: Tuple[str, int, int, int, bool], strategy: str) -> float:
...
def pack_all_blocks(
    pool: List[Tuple[str, int, int, int, bool]],
    ...
    pool_with_priority = [
        (
            ...
            dy,
            dz,
            compute_priority((name, dx, dy, dz, is_mand), priority_strategy),
        )
    ...
    if priority_strategy.startswith("two_phase"):
        ...
        # Phase 2
        optional_pool = [
            (n, dx, dy, dz, WP_MAP[n].profit_density)
            for n, dx, dy, dz, is_mand in pool
            if not is_mand
        ]
        ...
        for bname in sorted(
            packers.keys(), key=lambda n: packers[n].get_waste(), reverse=True
        ):
            pk = packers[bname]
            if pk.get_waste() <= 0:
                continue
            ...
            small = [
                (n, dx, dy, dz, pr)
                for n, dx, dy, dz, pr in remaining
                if min(dx, dy, dz) <= 30
            ]
            while small:
...
def destroy_and_repair(
    best_packers: Dict[str, BlockPacker],
    rng: random.Random,
    iterations: int = 500,
    ...
    for wp in WORKPIECES:
        ...
        for k in range(est_max):
            ori = rng.choice(oris)
            filling_pool.append((wp.name, ori[0], ori[1], ori[2], wp.profit_density))
    ...
    for it in range(iterations):
        ...
        if rng.random() < 0.3:
            destroy_idx = set(
                rng.sample(range(len(optional)), min(n_destroy, len(optional)))
            )
        else:
        ...
        kept_optional = [
            item for i, item in enumerate(optional) if i not in destroy_idx
        ]
        ...
        for bname in sorted(
            new_packers.keys(), key=lambda n: new_packers[n].get_waste(), reverse=True
        ):
            pk = new_packers[bname]
            if pk.get_waste() <= 0:
                continue
...
def solve(num_trials: int = 24, ils_iterations: int = 300):
    ...
    print(
        f"\n构造阶段: {num_trials} 次试验, 最优 {best_profit:,}, {construct_time:.1f}s"
    )
    ...
    if ils_iterations > 0:
        ...
        best_packers = destroy_and_repair(best_packers, ils_rng, ils_iterations)
...
def print_solution(sol):
    ...
    for wp in WORKPIECES:
        ...
        print(
            f"  {wp.name:<6} {wp.length:>3}×{wp.width:>3}×{wp.height:<6} "
    ...
    print(f"\n每块原材料")
    print(f"  {'块':<8} {'尺寸':<20} {'工件数':<7} {'已用体积':<14} {'利用率':<8}")
    for bname in sorted(sol["results"].keys()):
        d = sol["results"][bname]
        print(
            f"  {bname:<8} {d['dims'][0]}×{d['dims'][1]}×{d['dims'][2]:<8} "
